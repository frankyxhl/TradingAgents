Feature: Provider routing
  ...
  Scenario: ZAI provider uses Z.AI Coding API endpoint
    ...
    Then the client should use the Z.AI Coding API endpoint "https://api.z.ai/api/coding/paas/v4"

  Scenario: XAI provider uses xAI endpoint
    Given provider "xai"
    When creating an LLM client
    Then the client should use the Z.AI Coding API endpoint "https://api.x.ai/v1"
